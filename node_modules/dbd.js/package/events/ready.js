...
module.exports = async (client, Database) => {
	...
	if (client.options.fetchInvites) await new Promise(async resolve => {
		...
		while (i < guilds.length) {
			...
			; (async () => {
				...
				const invites = (await guilds[c].fetchInvites().catch(err => { }) || new Discord.Collection()).array()
	...
	console.log("Need help? Visit our support server! https://discord.gg/HMUfMXDQsV")
	...
	if (client.statuses.size) {
		...
		const f = async () => {
			...
			const t = await interpreter(client, {}, [], {
			    name: "status", 
			    code: status.text
			}, undefined, true) 
	...
	if (client.dbhToken) {
		...
		if (DBHError) {
			console.error("\x1b[31mDANBOT-HOSTING API: \x1b[0mFailed to Connect, " + DBHError + `.
    How to get API Key?
    ...
    3. Receive the Token from Bot DM and change the dbhToken in your main file to the new Token`);
			...
			console.log("\x1b[32mDANBOT-HOSTING API: \x1b[0mSuccessful Connection has been made to API")
		}
	}
