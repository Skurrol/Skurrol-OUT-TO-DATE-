const Parser = {
  ...
  $memberExists: "Checks if given user ID is in the server;$memberExists[userID;guildID (optional)]",
  ...
  $rubenGay:"Returns true or false",
  ...
  $awaitReaction: "Awaits an user reaction in this message;$awaitReaction[userFilter;time;bot message;reaction1,reaction2,...;command1,command2,...;error message;deleteMessageUponReact (yes/no) (optional)]",
  ...
  $getUserVar: "Gets an user variable value;$getUserVar[variable;userID (optional);guildID (optional)]",
  ...
  $setUserVar: "Sets a user variable value;$setUserVar[variable;value;userID (optional);guildID (optional)]",
  ...
  $queue: "Returns a queue of songs;$queue or $queue[page;amount;{number}) {title} by <@{userID}>;return current (yes or no) (optional)]",
  ...
  $playSong: "Plays a song from youtube. Support Playlist & URL;$playSong[song;leave vc time;defean (yes or no);leave when vc empty (yes/no);error]",
  ...
  $hasRole: "Checks if the user has the given role.;$hasRole[userID;roleID;guildID (optional)]",
  ...
  $playSpotify: "Play a Spotify playlist;$playSpotify[url;leave vc time; deafen;leave when vc empty (yes/no); useFilter (yes/no, default is yes);error]",
  ...
  $playSoundCloud: "Play SoundCloud Track URL;$playSoundCloud[url;soundcloud client id (optional);leave vc time;defean (yes or no);leave when vc empty (yes/no);error]",
  //1.3.0
...
*/
  ...
  $clearSongQueue: "Clear all songs in queue besides 1st song in queue;$clearSongQueue",
  ...
  $messageFlags: "Returns the message flags for this message",
	//2.0.0
	$moveSong: "Move song in the queue from x position to z, or remove it from queue if z is invalid;$moveSong[from;to (optional)]",
	$math: "Calculate Numbers instantly;$math[calculation]||$math[8*(7+9)]",
	$channelCooldown: "Sets a cooldown to current channel for current command;$channelCooldown[time;error message]",
	$timezone: "Set or return current timezone;$timezone or $timezone[timezone]",
	$moveUser: "Move a user from a voice channel or disconnect them;$moveUser[userID;channelID (optional);reason (optional)]",
	$deafenUser: "Deafen a user in voice channel;$deafenUser[userID;deafen (yes/no);reason (optional)]",
	$muteUser: "Mute a user in voice channel;$muteUser[userID;mute (yes/no);reason (optional)]",
	$editChannel: "Edit a guild channel data;$editChannel[channelID;categoryID/$default;name/$default;position/$default;nsfw/$default (yes/no);bitrate/$default;userLimit/$default;syncPermission (yes/no);reason (optional)]",
	...
	$giveRole: "Give a role to given user by using the role and user ID;$giveRole[userID;roleID]", 
	$takeRole: "Remove a role from given user by using the role and user ID;$takeRole[userID;roleID]", 
	...
	$channelPermissionsFor: "Returns the permissions for this user in a channel;$channelPermissionsFor[userID] or $channelPermissionsFor[channelID;userID]", 
	...
	$booster: "Returns raw function if the user is not boosting the official server",
	$userInfo: "Return the user invites info;$userInfo[option;userID (optional)]",
	$resetInvites: "Reset the guild or user invites;$resetInvites or $resetInvites[guildID;userID (optional)]",
  $seekTo: "Skip a certain amount of duration that the request want to skip (in Seconds);$seekTo[duration (seconds)]||$seekTo[7s] => 7 Seconds of Song Skipped", 
  ...
  $botLastMessageChannelID: "Returns the ID of the last channel the bot sent a message in (if any) (and cached)", 
  $botLastMessageID: "Returns the ID of the last message sent by the bot (if any) (and cached)", 
  ...
  $hasAnyRole: "Checks if the author or user have at least one of the provided role IDs;$hasAnyRole[roleID1;roleID2;...] or $hasAnyRole[userID;roleID1;roleID2;...]",
	...
	$serverExists: "Checks if your bot is in given guild ID.;$serverExists[guildID]",
	$songFilter: "Set an audio filter to current song;$songFilter[type:value;type:value;...]", 
	$createFile: "Creates a file containing text inside.;$createFile[text;name.extension]", 
	...
	//next update
	$getCollectionKey: "Gets a key value from given collection name.;$getCollectionKey[collection name;key]", 
	$deleteCollectionKey: "Deletes a key from given collection name.;$deleteCollectionKey[collection name;key]", 
	$setCollectionKey: "Sets a key value to given collection name. Last field will set a timer for this key and will be deleted after the timer is up.;$setCollectionKey[collection name;key;value;timer (optional)]", 
	$findCollectionKey: "Finds a key by using the key value, third field determines whether the input must be accurate to what the key value contains.;$findCollectionKey[collection name;key value]", 
	$getCooldownTime: "Gets a command cooldown time left for a type;$getCooldownTime[globalUser/user/server;command name;ID;return ms (yes/no) (optional)]", 
	$timeoutData: "Function that holds data for $timeout data parameters, this will be empty if used outside timeoutCommand codes.;$timeoutData[property]", 
	$emojiExists: "Checks if given emoji ID is available to the bot.;$emojiExists[emojiID]", 
	$serverEmojiExists: "Checks if given emoji ID is added to this guild.;$serverEmojiExists[emojiID]" , 
...
`, 
    ...
    $userRoleCount: "Return the role count of an user ;$userRoleCount or $userRoleCount[userID]", 
    ...
    $channelOverwrites: "Returns the channel permission overwrites;$channelOverwrites or $channelOverwrites[channelID;{mention} ({type}):\\nAllow: {allowed}\\nDeny: {denied};separator]",
    ...
    $emoji: "A compact function with 10 different functionalities!;$emoji[emojiID;option]",
    $guild: "A compact function with 21 different functionalities!;$guild[guildID;option]",
    $msg: "A compact function with 20 different functionalities;$msg[messageID;option]",
    $role: "A compact funcion with 16 different functionalities;$role[roleID;option]",
    $user: "A compact funcion with 12 different functionalities;$user[userID;option]",
    ...
    $webhook: "A compact funcion with 10 different functionalities;$webhook[hookID;option]",
    $let: "Assigns data to a temporary variable that can be changed and retrieved later in the command. Similar to variables but `$let` values only save per command execution.",
    $get: "Returns data from the `$let` function.. Similar to variables but `$get` values only save per command execution.",
    // Next Update 
