...
module.exports = async (d) => {
  ...
  if (!user)
    return d.error(`:x: Invalid user ID in 1st field of \`$user${inside}\`.`);

  let ch = d.message.channel.id;
  let result = option.toLowerCase();

  if (!result)
    return d.error(`:x: Missing option in 2nd field of \`$user${inside}\`.`);
  if (
    ![
      "avatar",
      "created",
    ...
    return d.error(`:x: Invalid option in 2nd field of \`$user${inside}\`.`);
