...
let ROLE;
try {
    ROLE = d.client.guilds.cache.find(g => g.roles.cache.get(id)).roles.cache.get(id)
} catch {
    ROLE = {guild:{}};
}
 ...
 if(!result) return d.error(`:x: Missing option in 2nd field of \`$role[${inside}]\`.`)
 if(![
    "created",
    "hex",
    "id",
    ...
    "usercount",
    "timestamp"
].includes(result)) return d.error(`:x: Invalid option in 2nd field of \`$role[${inside}]\`.`)
...
if(!ROLE & (result !== ("isdeleted"))) result = undefined
    ...
    switch (result) {
        ...
        case "created":
            ...
            break;
        case "timestamp":
            result = Object.entries(ms(Date.now() - ROLE.createdTimestamp)).map((x,y)=> {
