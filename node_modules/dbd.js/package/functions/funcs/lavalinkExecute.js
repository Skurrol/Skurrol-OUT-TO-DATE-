...
const timeoutFunction = function (d) {
  return () => {
    const lavalinkTimeout = global.LavalinkTimeout;
    ...
    if (player.state !== "PLAYING") {
      ...
      clearTimeout(lavalinkTimeout.get(d.message.guild.id));
      lavalinkTimeout.delete(d.message.guild.id);
    } else {
      clearTimeout(lavalinkTimeout.get(d.message.guild.id));
      lavalinkTimeout.delete(d.message.guild.id);
    }
...
module.exports = async (d) => {
  ...
  let response = "";
  global.LavalinkTimeout = global.LavalinkTimeout || new Map();
  ...
  if (!Player._events.trackStart) {
    Player.on("trackStart", () => {
      clearTimeout(global.LavalinkTimeout.get(d.message.guild.id));
      global.LavalinkTimeout.delete(d.message.guild.id);
      d.client.emit(
        "musicStart",
        d.client.lavalink.get(d.message.guild.id),
        null
      );
      Player.on("trackEnd", () => {
        clearTimeout(global.LavalinkTimeout.get(d.message.guild.id));
        ...
        if (!Player.queue.length) {
          ...
          global.LavalinkTimeout.set(d.message.guild.id, timeoutFunction());
        }
      });
    });
  }
    ...
    case "queue":
      {
        ...
        max = new Number(max);
        current = current.toLowerCase() === "yes";
        let y = max * page - max;

        if (current === true && y <= 0) y = 1;
