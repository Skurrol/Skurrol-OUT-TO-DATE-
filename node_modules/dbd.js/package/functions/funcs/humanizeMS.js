...
module.exports = d => {
  ...
  const r = code.split("$humanizeMS").length - 1
  const after = code.split("$humanizeMS")[r].after()
  if (after.inside === undefined) return {
    code
  }
  let [
    ms,
  ...
  for (const Duration of Object.keys(Durations).reverse()) {
    ...
    return {
      code:code.replaceLast(`$humanizeMS[${after.inside}]`, joinedArray)
    }
