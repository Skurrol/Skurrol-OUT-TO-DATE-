...
module.exports = async d => {
    ...
    if (EmojiFullFormat.test(fields[0])) fields[0] = fields[0].split(":")[2].replace(/(\<|\>)/, "");
 ...
 switch(result) {
    ...
    case "created":
        try {
         ...
         result = `${weekdays[date.getDay()]}, ${date.getHours() > 12 ? date.getHours() - 12 : date.getHours()}:${date.getMinutes()} ${isPM ? "PM" : "AM"} ${date.getDate()} ${months[date.getMonth()]} ${date.getFullYear()}`
        } catch (err){
          console.log(err)
           result = undefined
        }
