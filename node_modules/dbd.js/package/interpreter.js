		...
		if (!F || F.length === 0) { } else
			if (F) {
				...
				try {
					...
					const FFUNC = cachedFunc.has(FNAME) ? cachedFunc.get(FNAME) : require(`./functions/funcs/${FNAME}.js`)

					if (!cachedFunc.has(FNAME)) cachedFunc.set(FNAME, FFUNC)
