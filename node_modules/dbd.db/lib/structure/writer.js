...
class Writer extends EventEmitter {
		...
		super.on('exec', async () => {
			...
			if (this.run) {
				...
				await new Promise(resolve => {
					...
					setImmediate(() => { if (!this.run) resolve() })
				})
			}
			...
			const data = await this.stringify(this.db.cache.get(this.collection.displayName))
			...
			const dbm = await fs.promises.open(`${this.path}.bison`, 'r+').catch(err => { })
			...
			const stream = fs.createWriteStream(null, { fd: fh.fd, autoClose: false, encoding: 'utf8' })
			...
			stream.once('finish', async () => {
				...
				await fs.promises.rename(`${this.path}.tmp`, `${this.path}.bison`)
	...
	approximateSize(obj) {
		const sizeCache = this.db.sizeCache
		const objectCache = this.db.objectCache
		...
		function calc(d, t) {
			...
			switch(t) {
				...
				case 2:
					...
					if (d in sizeCache) bytes = sizeCache[d]
					else {
						bytes = `${d}`.length
				...
				case 4:
					...
					if (str in sizeCache) bytes = sizeCache[str]
					else {
